...
/* === VIVENTIUM START ===
 * Feature: Anthropic inline document payload size guard.
 * === VIVENTIUM END === */
const {
  isAnthropicProvider,
...
class BaseClient {
  ...
  async handleStartMethods(message, opts) {
    ...
    /* === VIVENTIUM NOTE ===
     * Feature: Durable assistant placeholder boundary for resumable agent replies.
     * Purpose: Ensure controller `onStart` persistence work completes before prompt building continues.
     * Added: 2026-03-24
     */
    if (typeof opts?.onStart === 'function') {
      const isNewConvo = !requestConvoId && parentMessageId === Constants.NO_PARENT;
      await opts.onStart(userMessage, responseMessageId, isNewConvo);
    }
    /* === VIVENTIUM NOTE END === */
  ...
  async loadHistory(conversationId, parentMessageId = null) {
    ...
    /* VIVENTIUM START
     * Purpose: Trace history loading + message metadata during background-cortex debugging.
     * Details: docs/requirements_and_learnings/05_Open_Source_Modifications.md#librechat-baseclient-load-history
     */
    logger.info(
  ...
  async addDocuments(message, attachments) {
    ...
    /* === VIVENTIUM START ===
     * Feature: Anthropic inline document payload size guard.
     * Why: Prevent provider-side 413 request_too_large by rejecting oversized inline PDF payloads early.
     * === VIVENTIUM END === */
    const provider = this.options.agent?.provider ?? this.options.endpoint;
    if (isAnthropicProvider(provider) && documentResult.documents?.length) {
      const validation = enforceAnthropicInlineDocumentLimits(documentResult.documents);
      if (!validation.valid) {
        logger.warn(
          `[BaseClient] Anthropic inline document guard skipped inline payload: reason=${validation.reason} totalMB=${Math.round(
            (validation.totalDocumentBytes ?? 0) / (1024 * 1024),
          )} (continuing with file context + file metadata only)`,
        );
        message.documents = undefined;
        return documentResult.files;
      }
      logger.debug('[BaseClient] Anthropic inline document payload validated', {
        documentCount: validation.documentCount,
        totalDocumentBytes: validation.totalDocumentBytes,
      });
    }
