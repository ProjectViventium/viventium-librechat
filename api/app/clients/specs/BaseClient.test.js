...
describe('BaseClient', () => {
  ...
  describe('sendMessage', () => {
    ...
    test('handleStartMethods waits for async onStart before resolving', async () => {
      const lifecycle = [];
      const onStartCalled = deferredPromise();
      const releaseOnStart = deferredPromise();

      let resolved = false;
      const startPromise = TestClient.handleStartMethods('Wait for start', {
        onStart: jest.fn(async () => {
          lifecycle.push('onStart');
          onStartCalled.resolve();
          await releaseOnStart.promise;
          lifecycle.push('afterResolve');
        }),
      });
      startPromise.then(() => {
        resolved = true;
      ...
      await onStartCalled.promise;
      expect(lifecycle).toEqual(['onStart']);
      expect(resolved).toBe(false);

      releaseOnStart.resolve();
      await startPromise;

      expect(lifecycle).toEqual(['onStart', 'afterResolve']);
