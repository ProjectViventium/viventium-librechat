 ...
 * Keep all repeated Agent spec setups on the same bounded helper and guard teardown.
 * Added: 2026-03-08
 */
...
describe('models/Agent', () => {
  ...
  describe('Agent CRUD Operations', () => {
    ...
    test('should preserve explicit Anthropic model names on create and read', async () => {
      const { agentId, authorId } = createTestIds();
    ...
    test('should align model_parameters.model with the explicit model on update', async () => {
      ...
      expect(updatedAgent.model).toBe('gpt-5.4');
      expect(updatedAgent.model_parameters.model).toBe('gpt-5.4');
      expect(updatedAgent.model_parameters.reasoning_effort).toBe('xhigh');

      const storedAgent = await Agent.findOne({ id: agentId }).lean();
      expect(storedAgent.model).toBe('gpt-5.4');
      expect(storedAgent.model_parameters.model).toBe('gpt-5.4');
    });
  ...
  describe('Agent Version History', () => {
    ...
    describe('Edge Cases', () => {
      ...
      test('should repair stale top-level agent fields when a matching version already exists', async () => {
        const agentId = `agent_${uuidv4()}`;
        const authorId = new mongoose.Types.ObjectId();

        await createAgent({
          id: agentId,
          name: 'Repair Test Agent',
          provider: 'openAI',
          model: 'gpt-4o',
          author: authorId,
        });
