...
describe('Conversation Operations', () => {
  ...
  describe('isTemporary conversation handling', () => {
    ...
    it('should handle minimum retention period (1 hour)', async () => {
      ...
      const result = await saveConvo(mockReq, mockConversationData);
      /* === VIVENTIUM START ===
       * Purpose: De-flake time-based retention assertions under load.
       * Why: `saveConvo` is async; under heavy CPU/IO the delta between beforeSave and the internal
       *      Date.now() used by saveConvo can exceed 1s, causing occasional failures.
       * Approach: Use afterSave as the upper bound anchor (same pattern as the "missing config" test).
       * Added: 2026-02-08
       */
      const afterSave = new Date();
      ...
      expect(actualExpirationTime.getTime()).toBeLessThanOrEqual(
        new Date(afterSave.getTime() + 1 * 60 * 60 * 1000 + 1000).getTime(),
      );
      /* === VIVENTIUM END === */
    });
