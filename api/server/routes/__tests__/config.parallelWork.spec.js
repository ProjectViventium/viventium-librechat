...
const mockParallelWorkDeploymentAvailableAsync = jest.fn();
const mockParallelWorkDeploymentAvailable = jest.fn(() => {
  throw new Error('synchronous Parallel Work validation reached the config route');
});
...
jest.mock('~/server/services/viventium/ViventiumOrchestrationMode', () => ({
  parallelWorkDeploymentAvailable: () => mockParallelWorkDeploymentAvailable(),
  parallelWorkDeploymentAvailableAsync: () => mockParallelWorkDeploymentAvailableAsync(),
}));
...
describe('startup config Parallel work availability', () => {
  ...
  test.each([false, true])(
    ...
    async (available) => {
      mockParallelWorkDeploymentAvailableAsync.mockResolvedValue(available);
      ...
      expect(mockParallelWorkDeploymentAvailableAsync).toHaveBeenCalledTimes(1);
      expect(mockParallelWorkDeploymentAvailable).not.toHaveBeenCalled();
    },
  );

  test('does not publish owner A readiness as generic startup availability', async () => {
    mockParallelWorkDeploymentAvailableAsync.mockResolvedValue(false);

    const response = await request(createApp()).get('/api/config').expect(200);

    expect(response.body.viventiumParallelWorkAvailable).toBe(false);
    expect(mockParallelWorkDeploymentAvailableAsync).toHaveBeenCalledTimes(1);
    expect(mockParallelWorkDeploymentAvailable).not.toHaveBeenCalled();
  });
});
