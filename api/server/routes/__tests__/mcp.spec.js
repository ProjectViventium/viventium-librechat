...
describe('MCP Routes', () => {
  ...
  describe('GET /:serverName/oauth/initiate', () => {
    ...
    it('should initiate OAuth flow using live registry config over stale flow metadata', async () => {
      const mockFlowManager = {
        getFlowState: jest.fn().mockResolvedValue({
          metadata: {
            serverUrl: 'https://test-server.com',
            oauth: { clientId: 'stale-client-id', redirect_uri: 'http://localhost:3080/callback' },
          },
        }),
      };
    ...
    it('should initiate OAuth flow when userId/flowId query params are omitted', async () => {
      ...
      MCPOAuthHandler.initiateOAuthFlow.mockResolvedValue({
        authorizationUrl: 'https://oauth.example.com/auth',
        flowId: 'generated-flow-id',
      });

      const response = await request(app).get('/api/mcp/test-server/oauth/initiate');
    ...
    it('should rewrite localhost redirect_uri to DOMAIN_SERVER callback', async () => {
  ...
  describe('GET /connection/status', () => {
    ...
    it('should dedupe persistent warmup across rapid status polls', async () => {
      ...
      try {
        ...
        getMCPSetupData.mockResolvedValue({
          mcpConfig: {
            'scheduling-cortex': { url: 'http://localhost:7110/mcp' },
          },
          appConnections: {},
          userConnections: {},
