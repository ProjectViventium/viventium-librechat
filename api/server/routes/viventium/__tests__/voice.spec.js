...
describe('/api/viventium/voice/chat', () => {
  ...
  test('decorates voice SSE with authoritative logical-turn metadata', async () => {
    const taskService = require('~/server/services/viventium/VoiceTaskService');
    const { GenerationJobManager } = require('@librechat/api');
    ...
    GenerationJobManager.getJob.mockResolvedValue({
      metadata: {
        ...
        interactionContext: {
          ...
          surface: 'voice',
          conversation_id: 'conv-voice-1',
          logical_turn_id: 'logical-voice-2',
          revision: 2,
    ...
    GenerationJobManager.subscribe.mockImplementationOnce((_streamId, onEvent, onDone) => {
      onEvent({ event: 'on_message_delta', data: { text: 'hello' } });
      onDone({ final: true });
      return { unsubscribe: jest.fn() };
    });
    const voiceRouter = require('../voice');
    ...
    const req = createMockReq({
      method: 'GET',
      url: '/api/viventium/voice/stream/stream-logical-voice',
      headers: { 'x-viventium-call-secret': 'secret' },
    });
    const res = createMockRes();
    ...
    expect(writes).toContain('"logical_turn_id":"logical-voice-2"');
    expect(writes).toContain('"revision":2');
    expect(writes).toContain('"surface":"voice"');
  });
  ...
  test("does not reveal another owner's voice stream existence", async () => {
    const { GenerationJobManager } = require('@librechat/api');
    GenerationJobManager.getJob.mockResolvedValueOnce({ metadata: { userId: 'other-owner' } });
    const voiceRouter = require('../voice');
    const app = createTestApp(voiceRouter);
    const req = createMockReq({
      method: 'GET',
      url: '/api/viventium/voice/stream/foreign-stream',
      headers: { 'x-viventium-call-secret': 'secret' },
    });
    const res = createMockRes();
