...
describe('tests for the new helper functions used by the MCP connection status endpoints', () => {
  ...
  describe('checkOAuthFlowStatus', () => {
    ...
    it('should treat a timed-out pending flow as stale (not failed)', async () => {
      const mockFlowState = {
        status: 'PENDING',
        createdAt: Date.now() - 11 * 60 * 1000, // 11 minutes ago (> 10m default TTL)
      };
      const mockFlowManager = { getFlowState: jest.fn(() => mockFlowState) };
      mockGetFlowStateManager.mockReturnValue(mockFlowManager);
      ...
      expect(result).toEqual({ hasActiveFlow: false, hasFailedFlow: false });
      expect(logger.debug).toHaveBeenCalledWith(
        expect.stringContaining('Found stale OAuth flow'),
        expect.objectContaining({
          flowId: mockFlowId,
        }),
      );
    ...
    it('should mark timed-out flow inactive when TTL exceeds default', async () => {
      const mockFlowState = {
        status: 'PENDING',
        createdAt: Date.now() - 11 * 60 * 1000, // 11 minutes ago (> 10m default TTL)
      };
    ...
    it('should detect active flow when status is PENDING and within TTL', async () => {
      const mockFlowState = {
        status: 'PENDING',
        createdAt: Date.now() - 6 * 60 * 1000, // 6 minutes ago (< 10m default TTL)
      };
      const mockFlowManager = { getFlowState: jest.fn(() => mockFlowState) };
      mockGetFlowStateManager.mockReturnValue(mockFlowManager);
    ...
    it('should respect FLOW_STATE_TTL_MINUTES when set', async () => {
      const previous = process.env.FLOW_STATE_TTL_MINUTES;
      try {
        process.env.FLOW_STATE_TTL_MINUTES = '1';

        const mockFlowState = {
          status: 'PENDING',
          createdAt: Date.now() - 70 * 1000, // 70 seconds ago (> 1m TTL)
        };
        const mockFlowManager = { getFlowState: jest.fn(() => mockFlowState) };
        mockGetFlowStateManager.mockReturnValue(mockFlowManager);
        ...
        if (previous == null) {
          delete process.env.FLOW_STATE_TTL_MINUTES;
        } else {
          process.env.FLOW_STATE_TTL_MINUTES = previous;
        }
      }
    });
  });
