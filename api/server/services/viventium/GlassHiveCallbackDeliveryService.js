...
async function recordSurfaceOutcomeBestEffort(originRef, state) {
  ...
  try {
    const { reconcileGlassHiveSurfaceDeliveryOutcome } = require('./GlassHiveCallbackBindingService');
    const current = await reconcileGlassHiveSurfaceDeliveryOutcome({ originRef });
    if (!current) throw Object.assign(new Error('glasshive_delivery_projection_owner_unavailable'), {
...
async function recordGlassHiveLinkedWebDelivery({
  body = {}, message, deliveryContext = {}, effectFence, effectSession,
}) {
  if (
  ...
  if (!effectFence) {
    const reference = await acceptedTerminalCallbackReference({
      body, deliveryContext, traceIdentity: callbackTraceIdentity(body, deliveryContext),
    });
    ...
    const lease = await acquireGlassHiveTerminalCallbackAcceptedOperationEffectLease({
      ResultModel: GlassHiveTerminalCallbackResult, reference, session: effectSession,
    });
    ...
    try {
      await fenceGlassHiveTerminalCallbackEffectTransaction({
        ResultModel: GlassHiveTerminalCallbackResult, lease, session: effectSession,
      });
      return await recordGlassHiveLinkedWebDelivery({
        body, message, deliveryContext, effectFence: lease, effectSession,
      });
    } finally {
  ...
  if (!ownerId || !conversationId || !originRef || !workRef ||
      !traceIdentity || traceIdentity.callbackRef !== callbackId) {
    throw new Error('glasshive_linked_presentation_identity_invalid');
  }
  const accepted = await queryLean(GlassHiveTerminalCallbackResult.findOne({
    _id: reference.resultKey, ownerId, originRef, workRef,
    workerId: normalizeText(body.worker_id), runId: normalizeText(body.run_id),
    acceptedOperationId: reference.acceptedOperationId,
    acceptedOperationGeneration: reference.generation,
    callbackId: reference.callbackId, resultDigest: reference.resultDigest,
    resultRevision: reference.resultRevision,
    effectLeaseId: normalizeText(effectFence.leaseId),
    effectLeaseGeneration: Number(effectFence.generation), effectLeaseExpiresAt: { $gt: nowDate() },
  }), effectSession);
  const stored = await queryLean(Message.findOne({
    user: ownerId, conversationId, messageId: normalizeText(message?.messageId),
  ...
  const deliveryKey = deliveryKeyFor({ ownerId, originRef, surface: 'librechat',
    callbackId, callbackMessageId: stored.messageId, event,
    attemptNumber: traceIdentity.attemptNumber });
  const now = nowDate();
  const row = await queryLean(ViventiumGlassHiveCallbackDelivery.findOneAndUpdate({
    deliveryKey, userId: ownerId, originRef, callbackMessageId: stored.messageId,
  }, { $setOnInsert: {
    deliveryKey, deliveryId: deliveryIdFor(deliveryKey), callbackId,
    callbackMessageId: stored.messageId, userId: ownerId, conversationId, originRef, workRef,
    workerId: normalizeText(body.worker_id), runId: normalizeText(body.run_id), event,
    surface: 'librechat', status: 'sent', sentAt: now, text: stored.text,
    projectionPendingAt: now, projectionNextAttemptAt: now,
    expiresAt: new Date(now.getTime() + DELIVERY_RETENTION_MS),
    ...terminalCallbackFields(reference),
  } }, { new: true, upsert: true, setDefaultsOnInsert: true, session: effectSession }), effectSession);
  const rows = await queryLean(ViventiumGlassHiveCallbackDelivery.find({ originRef, userId: ownerId, workRef,
    runId: normalizeText(body.run_id), ...terminalCallbackFields(reference) }, { status: 1 }), effectSession);
  const { recordGlassHiveSurfaceDeliveryOutcome } = require('./GlassHiveCallbackBindingService');
  await recordGlassHiveSurfaceDeliveryOutcome({ originRef,
    state: aggregateSurfaceState(new Set(rows.map((item) => item.status)), 'sent'),
    body: { ...body, callback_id: reference.callbackId, result_ended_at: accepted.resultEndedAt,
      result_digest: accepted.resultDigest, result_revision: accepted.resultRevision },
    effectFence, effectSession,
  });
  return row;
}

function pendingNeutralVoiceDelivery(row) {
  return row?.surface === 'voice' && row.status === 'pending' &&
    ['run.failed', 'run.cancelled', 'run.interrupted'].includes(row.event) &&
    row.traceIdentityVerified === true && !row.workerCompletionPresentation &&
    !row.claimId && !row.claimedAt && !row.dispatchPermitId && !row.fullText;
}
...
async function settleEndedCallNeutralDelivery(row, { effectFence, effectSession } = {}) {
  ...
  if (call?.status !== 'ended' || call.userId !== row.userId ||
      call.conversationId !== row.conversationId) return row;
  const reference = terminalCallbackReference({
    ...
    callbackId: row.terminalCallbackId, resultDigest: row.terminalCallbackResultDigest,
  ...
  const settle = async (session, fence) => {
    const message = await queryLean(Message.findOne({
      user: row.userId, conversationId: row.conversationId,
      messageId: row.callbackMessageId, isCreatedByUser: false,
    }), session);
    const metadata = message?.metadata?.viventium;
    if (metadata?.type !== 'glasshive_worker_callback' || metadata.status?.kind !== 'mission_status' ||
        metadata.status.state !== deliveryTerminalState(row.event) ||
        metadata.originRef !== row.originRef || metadata.workRef !== row.workRef ||
        metadata.workerId !== row.workerId || metadata.runId !== row.runId ||
        callbackRef(metadata.callbackId) !== row.callbackId ||
        metadata.voiceCallSessionId !== row.voiceCallSessionId || !normalizeText(message.text)) return row;
    const destinations = (metadata.configuredDestinations || []).filter((destination) =>
      destination.surface !== 'voice');
    if (!destinations.some((destination) => ['librechat', 'workbench'].includes(destination.surface))) return row;
    const now = nowDate();
    const updated = await queryLean(ViventiumGlassHiveCallbackDelivery.findOneAndUpdate({
      deliveryId: row.deliveryId, userId: row.userId, conversationId: row.conversationId,
      status: 'pending', surface: 'voice', claimId: { $in: ['', null] }, claimedAt: null,
      dispatchPermitId: { $in: ['', null] }, ...terminalCallbackFields(reference),
    }, { $set: { status: 'suppressed', suppressedAt: now, nextAttemptAt: null,
      lastError: 'call_ended_linked_text', projectionPendingAt: now, projectionNextAttemptAt: now,
    } }, { new: true, session }), session);
    if (!updated) return row;
    await recordGlassHiveLinkedWebDelivery({
      body: { callback_id: reference.callbackId, event: row.event,
        attempt_number: deliveryAttemptNumber(row) ?? null, worker_id: row.workerId, run_id: row.runId },
      message, deliveryContext: { ownerId: row.userId, conversationId: row.conversationId,
        originRef: row.originRef, workRef: row.workRef, destinations,
        traceIdentity: { callbackRef: row.callbackId, attemptNumber: deliveryAttemptNumber(row) ?? null } },
      effectFence: fence, effectSession: session,
    });
    return updated;
  };
  ...
  return runGlassHiveTerminalCallbackTransaction(async (session) => {
    const lease = await acquireGlassHiveTerminalCallbackAcceptedOperationEffectLease({
      ResultModel: GlassHiveTerminalCallbackResult, reference, session,
    });
    if (!lease) return row;
    try {
      await fenceGlassHiveTerminalCallbackEffectTransaction({
        ResultModel: GlassHiveTerminalCallbackResult, lease, session,
      });
      return await settle(session, lease);
    } finally {
      await releaseGlassHiveTerminalCallbackEffectLease({ ResultModel: GlassHiveTerminalCallbackResult, lease, session });
    }
  });
}
...
async function enqueueGlassHiveCallbackDelivery({
  ...
  if (destinations.length === 0) {
    const linked = await recordGlassHiveLinkedWebDelivery({ body, message, deliveryContext, effectFence, effectSession });
...
async function reconcileGlassHiveSurfaceDeliveryProjections({ limit = 25 } = {}) {
  ...
  const rows = await ViventiumGlassHiveCallbackDelivery.find({
    ...
    $or: [
      ...
      { projectionPendingAt: null, projectionAppliedAt: null },
      { surface: 'voice', status: 'pending', event: { $in: ['run.failed', 'run.cancelled', 'run.interrupted'] }, claimId: { $in: ['', null] }, claimedAt: null, dispatchPermitId: { $in: ['', null] }, $or: [{ projectionNextAttemptAt: null }, { projectionNextAttemptAt: { $lte: now } }] },
    ],
  })
  ...
  for (const [originRef, row] of origins) {
    ...
    if (pendingNeutralVoiceDelivery(row)) {
      await ViventiumGlassHiveCallbackDelivery.updateOne({ deliveryId: row.deliveryId, status: 'pending' },
