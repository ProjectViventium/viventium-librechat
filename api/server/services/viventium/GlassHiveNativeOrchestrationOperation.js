...
function verifyNativeOrchestrationOperation({
  ...
  /* A refreshed broker grant is new transport authority for the same signed occurrence. Once the
   * legacy token proves its stable bindings, converge it onto the same HMAC identity as the native
   * one-call path instead of letting ephemeral grant/token material create a sibling operation. */
  return commitNativeOrchestrationOperation({ grant, toolName: cleanToolName, args });
}

/**
 * Commit one provider-owned orchestration mutation without a model-visible prepare/confirm loop.
 * The authenticated conversation turn and canonical arguments already provide the stable occurrence
 * identity that the legacy opaque token manufactured. HMAC keeps the downstream operation id
 * Core-authored, while omitting the ephemeral grant id lets an exact lost-response retry survive a
 * short-lived broker-grant refresh.
 */
function commitNativeOrchestrationOperation({ grant, toolName, args = {} } = {}) {
  const cleanToolName = boundedString(toolName, 192);
  if (!isConversationOrchestrationMutationTool(cleanToolName)) {
    operationError('orchestration_operation_tool_unsupported');
  }
  ...
  const invocationId = `ghno_${crypto
    .createHmac('sha256', operationSecret())
    .update(stableJson(stableOccurrence), 'utf8')
    .digest('hex')}`;
  return {
    invocationId,
  ...
  return Object.freeze({
    ...
    properties: Object.freeze({
      ...
      [NATIVE_OPERATION_TOKEN_FIELD]: Object.freeze({
        ...
        maxLength: 8192,
        description:
          'Legacy Core-signed commit token. New trusted conversation turns commit in one exact call; when an older prepared response supplied a token, repeat the same arguments with it.',
      }),
    }),
  });
}
