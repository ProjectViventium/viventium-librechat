...
// Cancellation suppression is itself a terminal capability. Keep its bounded tombstone at least
// as long as any supported extended call/remote completion window so late work cannot reappear.
...
async function drainTaskPersistence() {
  while (pendingTaskPersistence.size > 0 && persistenceAvailable(ViventiumVoiceTask)) {
...
function scheduleTaskPersistenceDrain() {
  if (!taskPersistenceDrain) {
    taskPersistenceDrain = Promise.resolve()
      .then(drainTaskPersistence)
      .finally(() => {
...
async function persistVoiceTaskSuppression(task) {
  ...
  const expiresAtMs = Date.now() + ACTIVE_TASK_TTL_MS;
...
async function settleVoiceTaskCancellation(taskId, { confirmed, detail } = {}) {
  ...
  const previous = {
    state: task.state,
    ownerDeliveryPending: task.ownerDeliveryPending,
    expiresAtMs: task.expiresAtMs,
    sequence: task.sequence,
    updatedAt: task.updatedAt,
    current: { ...(task.current || {}) },
    ...
    events: [...task.events],
    cancellationPreparedEvent: task.cancellationPreparedEvent,
    suppressionEvent: task.suppressionEvent,
  };
  task.ownerDeliveryPending = false;
  task.state = confirmed ? 'cancelled_confirmed' : 'cancelled_unenforceable';
  ownerAdapters.delete(ownerAdapterKey(task.owner?.kind, task.taskId));
  task.expiresAtMs = Date.now() + TERMINAL_TASK_TTL_MS;
  const event = nextEvent(task, {
    type: 'state',
  ...
  try {
    ...
    task.ownerDeliveryPending = previous.ownerDeliveryPending;
    task.expiresAtMs = previous.expiresAtMs;
    task.sequence = previous.sequence;
    task.updatedAt = previous.updatedAt;
    task.current = previous.current;
    task.lastEvent = previous.lastEvent;
    task.events = previous.events;
    task.cancellationPreparedEvent = previous.cancellationPreparedEvent;
    task.suppressionEvent = previous.suppressionEvent;
    throw error;
  }
  ownerAdapters.delete(ownerAdapterKey(task.owner?.kind, task.taskId));
  publishTaskEvent(task, event);
...
function isVoiceTaskSuppressed(taskId) {
  ...
  if (tombstone && tombstone.expiresAtMs > Date.now()) return true;
