...
describe('GlassHiveConversationProviderService', () => {
  ...
  test('removes native broker bearer headers from a presentation-only provider lane', () => {
    const targetAgent = {
      model_parameters: {
        configuration: {
          defaultHeaders: {
  ...
  test('projects Main orchestration separately and never adds it to inheritable worker host tools', async () => {
  ...
  test('denies peer orchestration to a mission/root Agent even when it forges tool names', () => {
    const missionRoot = {
      id: 'mission-root',
      toolRegistry: new Map([
        [
