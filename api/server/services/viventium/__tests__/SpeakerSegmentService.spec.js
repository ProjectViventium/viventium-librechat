...
describe('SpeakerSegmentService', () => {
  ...
  test('scopes a persisted guest tombstone by stable participant identity across reconnect', () => {
    const ownerInput = {
      version: 1,
      segmentId: 'segment-owner-after-reconnect',
      turnId: 'turn-owner-after-reconnect',
      sequence: 10,
      revision: 0,
      text: 'verified owner speech after reconnect',
      isFinal: true,
      speaker: {
        key: 'track:owner',
        label: 'Owner',
    ...
    const guestInput = {
      ...ownerInput,
      segmentId: 'segment-guest-after-reconnect',
      speaker: {
        key: 'track:guest',
        label: 'Guest',
        source: 'hybrid',
    ...
    const [owner] = normalizeSpeakerSegments([ownerInput], context);
    const [guest] = normalizeSpeakerSegments([guestInput], context);
    const [missingIdentity] = normalizeSpeakerSegments(
      [
        {
          ...ownerInput,
          segmentId: 'segment-missing-identity',
          speaker: { ...ownerInput.speaker, participantIdentity: '' },
        },
      ],
      context,
    );

    expect(owner.speaker).toMatchObject({
      attribution: 'verified',
      actorTrust: 'owner_participant',
    });
    expect(
      voiceTurnAuthority([owner], {
        speakerAttributionState: 'shared_mic_unverified',
        sharedParticipantIdentities: ['guest'],
      }),
    ).toMatchObject({ actorTrust: 'owner_participant', canAuthorizeSideEffects: true });
    expect(guest.speaker).toMatchObject({
      attribution: 'unverified',
      actorTrust: 'shared_mic_unverified',
    });
    expect(missingIdentity.speaker).toMatchObject({
      attribution: 'unverified',
      actorTrust: 'shared_mic_unverified',
    });
  });
  ...
  test('keeps the highest payload revision under concurrent and reordered persistence', async () => {
    ...
    const advanceEvidenceEpoch = jest
      .spyOn(ViventiumCallSession, 'updateOne')
      .mockResolvedValue({ matchedCount: 1 });
    const invalidateMemoryFinalization = jest
    ...
    create.mockRestore();
    findOne.mockRestore();
    advanceEvidenceEpoch.mockRestore();
    invalidateMemoryFinalization.mockRestore();
  });
  ...
  test('persists a monotonic shared-mic session tombstone that can never revert', async () => {
    ...
    const findOneAndUpdate = jest
      ...
      .mockImplementation((query, update) => ({
        lean: async () => {
          const scopedParticipants = update.$addToSet?.speakerSharedParticipantIdentities?.$each;
          if (Array.isArray(scopedParticipants)) {
            const scopedTracks = update.$addToSet?.speakerSharedTrackSids?.$each || [];
            stored = {
              ...stored,
              ...(update.$set || {}),
              speakerSessionRevision: Math.max(
                Number(stored.speakerSessionRevision) || 0,
                Number(update.$max?.speakerSessionRevision) || 0,
              ),
              speakerSharedTrackSids: [
                ...new Set([...(stored.speakerSharedTrackSids || []), ...scopedTracks]),
              ].sort(),
              speakerSharedParticipantIdentities: [
                ...new Set([
                  ...(stored.speakerSharedParticipantIdentities || []),
                  ...scopedParticipants,
                ]),
              ].sort(),
            };
    ...
    expect(invalidateMemoryFinalization).toHaveBeenCalledWith(
      { 'metadata.viventium.callSessionId': 'call-tombstone' },
      { $unset: { 'metadata.viventium.memoryFinalization': '' } },
    );
    expect(invalidateMemoryFinalization).toHaveBeenCalledTimes(4);
    expect(invalidateMemoryFinalization.mock.invocationCallOrder.at(-1)).toBeGreaterThan(
  ...
  test('waits for an active memory-finalization lease before advancing speaker evidence', async () => {
    const stored = {
      callSessionId: 'call-finalization-lease',
      expiresAt: new Date('2099-01-01T00:00:00.000Z'),
      speakerSessionRevision: 0,
      speakerAttributionState: 'single_speaker',
      speakerDetectedAt: new Date('2026-08-09T10:00:00.000Z'),
      memoryFinalizationLeaseId: 'lease-in-progress',
      memoryFinalizationLeaseExpiresAt: new Date('2099-01-01T00:00:00.000Z'),
    };
    const findOne = jest.spyOn(ViventiumCallSession, 'findOne').mockImplementation(() => ({
      lean: async () => ({ ...stored }),
      select() {
    ...
    expect(advanceEvidenceEpoch).toHaveBeenCalledTimes(3);
    expect(invalidateMemoryFinalization).toHaveBeenCalledTimes(2);
  ...
  test('unions stable shared participant identities at one revision and keeps track evidence', async () => {
    let stored = {
      callSessionId: 'call-track-set',
      expiresAt: new Date('2099-01-01T00:00:00.000Z'),
      speakerSessionRevision: null,
      speakerAttributionState: null,
      speakerDetectedAt: null,
    };
    const findOne = jest.spyOn(ViventiumCallSession, 'findOne').mockImplementation(() => ({
      lean: async () => ({ ...stored }),
    }));
    const findOneAndUpdate = jest
      ...
      .mockImplementation((query, update) => ({
        lean: async () => {
          const participantIdentities = update.$addToSet?.speakerSharedParticipantIdentities?.$each;
          if (Array.isArray(participantIdentities)) {
            const trackSids = update.$addToSet?.speakerSharedTrackSids?.$each || [];
            if (
              participantIdentities.every((identity) =>
                (stored.speakerSharedParticipantIdentities || []).includes(identity),
              )
            ) {
              return null;
            }
            stored.speakerSharedTrackSids = [
              ...new Set([...(stored.speakerSharedTrackSids || []), ...trackSids]),
            ].sort();
            stored.speakerSharedParticipantIdentities = [
              ...new Set([
                ...(stored.speakerSharedParticipantIdentities || []),
            ...
            stored.speakerSessionRevision = Math.max(
              Number(stored.speakerSessionRevision) || 0,
              Number(update.$max?.speakerSessionRevision) || 0,
            );
            return { ...stored };
          ...
          if (
            Number(stored.speakerSessionRevision) >= Number(update.$set?.speakerSessionRevision)
          ) {
            return null;
          }
          Object.assign(stored, update.$set || {});
    ...
    const first = await persistSpeakerSessionState({
      callSessionId: 'call-track-set',
      state: stateFor('guest-track-1', 'guest-1'),
    });
    const second = await persistSpeakerSessionState({
      callSessionId: 'call-track-set',
      state: stateFor('guest-track-2', 'guest-2'),
    });
    const retry = await persistSpeakerSessionState({
      callSessionId: 'call-track-set',
    ...
    expect(retry.state.sharedTrackSids).toEqual(['guest-track-1', 'guest-track-2']);
    expect(stored.speakerSharedTrackSids).toEqual(['guest-track-1', 'guest-track-2']);
    expect(retry.state.sharedParticipantIdentities).toEqual(['guest-1', 'guest-2']);
    findOne.mockRestore();
    findOneAndUpdate.mockRestore();
    advanceEvidenceEpoch.mockRestore();
    invalidateMemoryFinalization.mockRestore();
