const {
  conversationRecallSourceUrl,
  renderConversationRecallTurn,
} = require('../conversationRecallSource');
...
describe('conversation recall source addresses', () => {
  it('uses the configured current instance and base path without query or fragment state', () => {
    expect(
      conversationRecallSourceUrl('source/with space', 'https://chat.example.test/app?x=1#draft'),
  ...
  it.each([undefined, '', 'javascript:alert(1)', 'https://user:password@example.test/'])(
    'does not fabricate or expose a source address from invalid configuration %s',
    (clientUrl) => {
