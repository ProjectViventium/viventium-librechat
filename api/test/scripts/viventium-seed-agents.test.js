...
describe('viventium-seed-agents', () => {
  ...
  test('audits every published predecessor and proves unchanged versus edited migration behavior', () => {
    ...
    expect(tracked.public_lock_revision_count).toBe(170);
    expect(tracked.invalid_predecessors).toHaveLength(3);
    expect(tracked.migrations).toHaveLength(30);
    expect(tracked.migrations.flatMap((item) => item.predecessor_source_refs)).toHaveLength(91);
