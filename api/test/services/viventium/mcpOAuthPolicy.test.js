...
describe('mcpOAuthPolicy', () => {
  ...
  describe('getMcpOAuthWaitDecision', () => {
    test('never waits for Telegram surface', () => {
      ...
      const decision = getMcpOAuthWaitDecision(req);
    ...
    test('intent mode waits only when tool intent exists on web surface', () => {
      const noIntent = getMcpOAuthWaitDecision(buildReq('what should be my priority based on memory'));
      expect(noIntent.surface).toBe('web');
      expect(noIntent.mode).toBe('intent');
      expect(noIntent.waitForOAuth).toBe(false);

      const withIntent = getMcpOAuthWaitDecision(buildReq('check my inbox'));
    ...
    test('always mode forces wait on web/voice', () => {
      ...
      const webDecision = getMcpOAuthWaitDecision(buildReq('hi'));
    ...
    test('never mode forces non-blocking behavior on web/voice', () => {
      ...
      const webDecision = getMcpOAuthWaitDecision(buildReq('check my inbox'));
