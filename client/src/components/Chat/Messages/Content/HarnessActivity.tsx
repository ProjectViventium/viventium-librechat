...
export default function HarnessActivity({
  ...
  const isFallbackRecovery = event === 'fallback-recovery';

  return (
    <details
      data-viventium-harness-activity="true"
      className="bg-surface-secondary/50 group my-2 rounded-lg border border-border-light px-3 py-2"
    >
      <summary className="flex cursor-pointer list-none items-center gap-2 text-sm text-text-secondary">
        ...
        <span className="font-medium">
          {isFallbackRecovery
            ? localize('com_ui_model_fallback_used')
            : localize('com_ui_harness_activity')}
        </span>
        <ChevronDown
