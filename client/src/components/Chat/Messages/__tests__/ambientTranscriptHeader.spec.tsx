...
jest.mock('librechat-data-provider/react-query', () => ({
  useUpdateFeedbackMutation: () => ({ mutate: jest.fn() }),
}));
jest.mock('~/Providers', () => ({
  useChatContext: () => ({
    conversation: { endpoint: 'agents', agent_id: 'active-agent' },
    regenerate: jest.fn(),
  }),
...
jest.mock('~/hooks', () => ({
  useLocalize: () => (key: keyof typeof mockTranslations, args?: { 0?: string }) =>
    (mockTranslations[key] ?? key).replace('{{0}}', args?.[0] ?? ''),
  useAttachments: () => ({}),
  useMessageHelpers: () => ({
    agent: { name: 'Configured assistant' },
...
const base: TMessage = {
  ...
  text: 'Retained call speech',
  sender: 'Unknown',
};
const header = (message: TMessage, path: string) => {
  if (path === 'content') {
    render(<MessageParts message={message} currentEditId={null} />);
    return screen.getByRole('heading').textContent;
  }
  return renderHook(() => useMessageActions({ message, currentEditId: null })).result.current
    .messageLabel;
};
describe.each(['content', 'legacy'])('%s ambient speaker header', (path) => {
  it.each(['listen_only_transcript', 'voice_ambient_transcript'])(
    'keeps typed %s speaker provenance',
    (type) => {
      expect(
        header({ ...base, metadata: { viventium: { type, speakerLabel: 'Unknown' } } }, path),
      ).toBe('Unknown · Call transcript');
    },
  );
  it('retains a generic speaker label', () => {
    expect(
      header(
        {
          ...base,
          metadata: { viventium: { type: 'voice_ambient_transcript', speakerLabel: 'Speaker 2' } },
        },
        path,
      ),
    ).toBe('Speaker 2 · Call transcript');
  });
      ...
      header(
        {
          ...base,
          metadata: {
            viventium: { type: 'listen_only_transcript', speakerLabel: { name: 'User' } },
          },
        },
        path,
      ),
  ...
  it.each([undefined, { viventium: { type: 'regular_reply', speakerLabel: 'Unknown' } }])(
    'keeps ordinary assistant identity',
    (metadata) => {
      ...
      header(
        {
          ...
          metadata: { viventium: { type: 'listen_only_transcript', speakerLabel: 'Unknown' } },
        },
        path,
      ),
    ).toBe(mockTranslations.com_user_message);
  });
});
