...
export default function ModelPanel({
  ...
  const fallbackModelCapability = useMemo(
    () => providerCapability?.models?.find((item) => item.id === glassHiveFallbackModel),
  ...
  useEffect(() => {
    ...
    const fallbackSelectionValid =
      Boolean(glassHiveFallbackModel) &&
      glassHiveFallbackModel !== model &&
      Boolean(fallbackModelCapability);
    if (glassHiveFallbackModel && !fallbackSelectionValid) {
      setValue('glasshive_options.fallback_model', undefined, { shouldDirty: true });
      setValue('glasshive_options.fallback_reasoning_effort', undefined, {
        shouldDirty: true,
      });
    } else if (fallbackSelectionValid) {
      const fallbackEffortChoices = fallbackModelCapability?.effortChoices ?? [];
      if (
        fallbackModelCapability?.recommendedEffort &&
        (!glassHiveFallbackEffort || !fallbackEffortChoices.includes(glassHiveFallbackEffort))
      ) {
        setValue(
          'glasshive_options.fallback_reasoning_effort',
          fallbackModelCapability.recommendedEffort,
          { shouldDirty: true },
        );
      }
    } else if (glassHiveFallbackEffort) {
      setValue('glasshive_options.fallback_reasoning_effort', undefined, {
        shouldDirty: true,
      });
    }
  }, [
    fallbackModelCapability,
    fallbackModelCapability?.effortChoices,
    fallbackModelCapability?.recommendedEffort,
    glassHiveOptions?.access,
    glassHiveFallbackEffort,
