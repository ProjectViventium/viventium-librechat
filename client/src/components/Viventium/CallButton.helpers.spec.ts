...
import {
  ...
  parseCallTaskContinuationState,
  renderPendingCallWindow,
  ...
  shouldContinueCallTaskPolling,
} from './CallButton';

describe('CallButton trust and failure helpers', () => {
  test('renders a branded public-safe connecting shell before async call preflight', () => {
    const popupDocument = document.implementation.createHTMLDocument('');
    const close = jest.fn();
    const openerFocus = jest.fn();
    const popup = {
    ...
    renderPendingCallWindow(popup, 'connecting');
    expect(popupDocument.title).toBe('Connecting to Viventium…');
    expect(popupDocument.body.textContent).toContain('Checking the configured voice route');
    expect(popupDocument.body.textContent).not.toContain('callSessionId');

    renderPendingCallWindow(popup, 'error', 'The configured voice provider is unavailable.');
    expect(popupDocument.querySelector('[role="alert"]')?.textContent).toContain(
      'configured voice provider',
    );
    (popupDocument.querySelector('button') as HTMLButtonElement).click();
  ...
  test('obeys server-owned continuation truth beyond a five-second late-child gap', () => {
    const monitoring = parseCallTaskContinuationState({
      version: 1,
      status: 'monitoring',
      hasActive: false,
      observedAt: '2026-08-09T12:00:06.000Z',
      quietUntil: '2026-08-09T12:15:00.000Z',
      nextPollAfterMs: 5000,
    });
    expect(shouldContinueCallTaskPolling(monitoring)).toEqual({
      shouldContinue: true,
      nextPollAfterMs: 5000,
    });

    const lateChild = parseCallTaskContinuationState({
      version: 1,
      status: 'active',
      hasActive: true,
      observedAt: '2026-08-09T12:00:07.000Z',
      quietUntil: '2026-08-09T12:15:07.000Z',
      nextPollAfterMs: 1500,
    });
    expect(shouldContinueCallTaskPolling(lateChild).shouldContinue).toBe(true);

    const quiescent = parseCallTaskContinuationState({
      version: 1,
      status: 'quiescent',
      hasActive: false,
      observedAt: '2026-08-09T12:15:08.000Z',
      quietUntil: '2026-08-09T12:15:07.000Z',
      nextPollAfterMs: null,
    });
    expect(shouldContinueCallTaskPolling(quiescent)).toEqual({
      shouldContinue: false,
      nextPollAfterMs: null,
    });
    expect(() => parseCallTaskContinuationState({ ...monitoring, status: 'quiescent' })).toThrow(
      'Malformed call task continuation state',
    );
  });
  ...
  test('retrieves every stable continuation page so old active and newest completed work survive restart', async () => {
    const continuation = {
      ...
      quietUntil: '2026-08-09T12:15:01.000Z',
      nextPollAfterMs: 1500,
    };
