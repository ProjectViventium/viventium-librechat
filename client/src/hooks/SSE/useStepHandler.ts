...
export default function useStepHandler({
  ...
  const stepHandler = useCallback(
    ({ event, data }: TStepEvent, submission: EventSubmission) => {
      ...
      if (event === 'on_run_step') {
      ...
      } else if (event === 'on_cortex_followup') {
        const followUp = data as {
          runId?: string;
          messageId?: string;
          parentMessageId?: string;
          conversationId?: string;
        ...
        const nextConversationId = followUp.conversationId ?? fallbackConversationId;
        const nextParentMessageId =
          typeof followUp.parentMessageId === 'string' &&
          followUp.parentMessageId.length > 0 &&
          followUp.parentMessageId !== followUp.messageId
            ? followUp.parentMessageId
            : undefined;
        ...
        if (existingIdx >= 0) {
          ...
          updated[existingIdx] = {
            ...
            parentMessageId: nextParentMessageId ?? existing.parentMessageId,
            text: followUpText,
            unfinished: false,
            content:
              existing.content && existing.content.length > 0
                ? existing.content
                : [{ type: ContentTypes.TEXT, text: followUpText }],
            metadata: {
        ...
        const followUpMessage = {
          ...
          parentMessageId: nextParentMessageId ?? userMessage.messageId,
          conversationId: nextConversationId,
          isCreatedByUser: false,
          sender: 'Assistant',
          text: followUpText,
          content: [{ type: ContentTypes.TEXT, text: followUpText }],
          metadata: {
            viventium: {
              type: 'cortex_followup',
              parentRunId: followUp.runId,
