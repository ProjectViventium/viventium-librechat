/* === VIVENTIUM START === Shared public-safe GlassHive callback evidence sanitization. === */
...
export function isSafeGlassHiveActionUrl(value: unknown = ''): boolean {
  try {
...
export function sanitizeGlassHiveCallbackText(
  ...
  let text = String(value || '')
    .trim()
    .replace(SAFE_GLASSHIVE_LINK_PATTERN, (match: string, url: string) => {
  ...
  text = text
    ...
    .replace(
      ...
      (_match: string, label: string) => `${label}=[secret]`,
    )
