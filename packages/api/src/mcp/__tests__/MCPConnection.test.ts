...
describe('extractSSEErrorMessage', () => {
  ...
  describe('connection errors', () => {
    it('should classify streamable-http terminated side-channel disconnects as transient', () => {
      const error = { message: 'SSE stream disconnected: TypeError: terminated' };
      const result = extractSSEErrorMessage(error);

      expect(result.message).toContain('SSE stream side-channel disconnected');
      expect(result.isProxyHint).toBe(false);
      expect(result.isTransient).toBe(true);
    });

    it('should classify streamable-http abort side-channel disconnects as transient', () => {
      const error = { message: 'SSE stream disconnected: AbortError: This operation was aborted' };
      const result = extractSSEErrorMessage(error);
