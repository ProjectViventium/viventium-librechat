...
describe('OAuthReconnectionManager', () => {
  ...
  describe('reconnectServers', () => {
    ...
    /* === VIVENTIUM START ===
     * Purpose: We should reconnect even when the access token is expired if a still-valid refresh token exists.
     * The reconnect path will refresh during connect rather than forcing the user to re-authenticate.
     */
    it('should reconnect servers when access token is expired but refresh token is valid', async () => {
      const userId = 'user-123';
      const oauthServers = new Set(['server1']);
      (mockRegistryInstance.getOAuthServers as jest.Mock).mockResolvedValue(oauthServers);

      // server1: access token expired, refresh token valid
      tokenMethods.findToken.mockImplementation(async ({ identifier, type }) => {
        if (type === 'mcp_oauth' && identifier === 'mcp:server1') {
          return {
            userId,
            identifier,
            expiresAt: new Date(Date.now() - 3600000), // 1 hour ago
        ...
        if (type === 'mcp_oauth_refresh' && identifier === 'mcp:server1:refresh') {
          return {
            userId,
            identifier,
            expiresAt: new Date(Date.now() + 3600000), // 1 hour from now
