...
function normalizeSourceFiles(
  ...
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    ...
    const fileId = String(candidate?.file_id || '')
      .trim()
      .slice(0, 256);
...
export function normalizeSourceSegmentsWithOverflow(
  ...
  for (const candidate of Array.isArray(candidates) ? candidates : []) {
    const sourceEventId = String(candidate?.source_event_id || '')
      .trim()
      .slice(0, 160);
    ...
    const suppliedDigest = String(candidate.original_sha256 || '')
      .trim()
      .toLowerCase();
    const sourceMessageId = String(candidate.source_message_id || '')
      .trim()
      .slice(0, 256);
    const sourceParentMessageId = String(candidate.source_parent_message_id || '')
      .trim()
      .slice(0, 256);
