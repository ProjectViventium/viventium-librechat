...
describe('web.ts', () => {
  ...
  describe('loadWebSearchAuth', () => {
    ...
    it('should check all services if none are specified', async () => {
      ...
      expect(result.authResult.rerankerType).toBeUndefined();
      const rerankerAuthType = result.authTypes.find(
        ([category]) => category === 'rerankers',
      )?.[1];
      expect(rerankerAuthType).toBe(AuthType.SYSTEM_DEFINED);
    });
  ...
  describe('loadWebSearchAuth with specific services', () => {
    ...
    it('should accept resolved literal provider and scraper config values', async () => {
      const literalConfig = {
        searchProvider: 'searxng' as SearchProviders,
        scraperProvider: 'firecrawl' as ScraperProviders,
        searxngInstanceUrl: 'http://localhost:8082',
        firecrawlApiKey: 'fc-local-key',
        firecrawlApiUrl: 'http://localhost:3003',
        firecrawlVersion: 'v2',
        safeSearch: SafeSearchTypes.MODERATE,
      } as TCustomConfig['webSearch'];
      ...
      const result = await loadWebSearchAuth({
        userId,
        webSearchConfig: literalConfig,
        loadAuthValues: mockLoadAuthValues,
      });

      expect(result.authenticated).toBe(true);
      expect(result.authResult.searchProvider).toBe('searxng');
      expect(result.authResult.searxngInstanceUrl).toBe('http://localhost:8082');
      expect(result.authResult.scraperProvider).toBe('firecrawl');
      expect(result.authResult.firecrawlApiKey).toBe('fc-local-key');
      expect(result.authResult.firecrawlApiUrl).toBe('http://localhost:3003');
      expect(result.authResult.rerankerType).toBeUndefined();
      expect(mockLoadAuthValues).not.toHaveBeenCalled();
    });

    it('should treat rerankers as optional when no rerankerType is configured', async () => {
      const configWithoutReranker = {
        searchProvider: 'searxng' as SearchProviders,
        scraperProvider: 'firecrawl' as ScraperProviders,
        searxngInstanceUrl: '${SEARXNG_INSTANCE_URL}',
        firecrawlApiKey: '${FIRECRAWL_API_KEY}',
      ...
      mockLoadAuthValues.mockImplementation(({ authFields }) => {
        ...
        authFields.forEach((field: string) => {
          result[field] =
            field === 'FIRECRAWL_API_URL' ? 'http://localhost:3003' : 'test-api-key';
        });
        return Promise.resolve(result);
      });
      ...
      const result = await loadWebSearchAuth({
        userId,
        webSearchConfig: configWithoutReranker,
        loadAuthValues: mockLoadAuthValues,
      });
