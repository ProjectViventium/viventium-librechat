...
describe('replaceSpecialVars', () => {
  ...
  /* === VIVENTIUM START ===
   * Feature: Timezone-aware current_datetime replacement tests
   * Purpose: Validate timezone rendering and fallback behavior.
   * Added: 2026-02-01
   */
  test('should honor a provided timezone for {{current_datetime}}', () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date('2026-02-01T21:09:00Z'));
    ...
    expect(result).toBe('Now is 2026-02-01 16:09:00 (0)');
    jest.useRealTimers();
  });

  test('should fall back when provided timezone is invalid', () => {
    const result = replaceSpecialVars({
      text: 'Now is {{current_datetime}}',
      timeZone: 'Not/AZone',
    });
