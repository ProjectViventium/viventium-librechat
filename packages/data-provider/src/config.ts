...
export const interfaceSchema = z
  .object({
     ...
     * Purpose: Allow setting a default agent via interface config (used by Viventium surfaces/entrypoints).
     * Added: 2026-02-07
     */
...
export const memorySchema = z.object({
  ...
  /* === VIVENTIUM START ===
   * Feature: Per-key memory budgets and maintenance threshold
   *
   * Purpose:
   * - Keep one memory key from monopolizing the shared memory budget.
   * - Trigger deterministic maintenance before the global store hard-fails.
   *
   * Added: 2026-03-09
   * === VIVENTIUM END === */
...
/* VIVENTIUM START
 * Purpose: Extend config schema for background cortex activation formatting.
 * Details: docs/requirements_and_learnings/05_Open_Source_Modifications.md#librechat-config-schema
 */
const backgroundCortexActivationFormatSchema = z.object({
...
export enum CacheKeys {
  ...
  TOOLS = 'TOOLS',
  /**
   * Key for the tool namespace cache.
   */
...
>(
  ...
  const directConfig = endpointsConfig[endpoint];
  if (directConfig) {
    return directConfig[property];
  }

  const normalizedEndpoint = normalizeEndpointName(String(endpoint));
  const matchedKey = Object.keys(endpointsConfig).find(
    (key) => normalizeEndpointName(key) === normalizedEndpoint,
  );
  const config = matchedKey ? endpointsConfig[matchedKey] : undefined;
  if (!config) {
    return undefined;
  }
