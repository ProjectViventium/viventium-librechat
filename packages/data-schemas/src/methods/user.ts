...
export function createUserMethods(mongoose: typeof import('mongoose')) {
   ...
   * Purpose: Persist the canonical account mode without replacing other user preferences.
   * === VIVENTIUM END === */
  async function updateUserViventiumOrchestrationPreferences(
    ...
    if (typeof viventiumOrchestrationPreferences.knownWork === 'boolean') {
      ...
      updateSet['personalization.parallel_work_known'] =
        viventiumOrchestrationPreferences.knownWork;
    }
  ...
  /** Read the durable positive-work fence before an authoritative remote roster observation. */
  async function getUserParallelWorkKnownEpoch(userId: string): Promise<number | null> {
    ...
    // Existing users predate the fence field. Missing durable state is epoch zero; no migration
    // write is required before the first positive observation or authoritative empty CAS.
    return Number.isSafeInteger(epoch) && Number(epoch) >= 0 ? Number(epoch) : 0;
  }

  /** Atomically publish positive work and advance the cross-process empty-observation fence. */
  ...
  /**
   * Clear the hint only if no positive observation advanced the durable fence since the caller's
   * pre-roster read. Legacy documents with no epoch field are exactly equivalent to epoch zero.
   */
