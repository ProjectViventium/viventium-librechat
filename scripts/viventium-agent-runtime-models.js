...
function canonicalBuiltInMainModelParameters(agentId, provider) {
  const normalizedProvider = normalizeProvider(provider);
  if (agentId !== MAIN_AGENT_ID || normalizedProvider !== GLASSHIVE_MAIN_PROVIDER) {
    return null;
  }
  return {
    reasoning_effort: GLASSHIVE_MAIN_REASONING_EFFORT,
  };
}
...
function rewriteAgentForRuntime(agent, { env = process.env } = {}) {
  ...
  if (envMap) {
    const assignment = shouldPreserveExplicitGlassHiveMain(rewritten, { env })
      ? {
    ...
    if (assignment) {
      ...
      const canonicalMainParameters = canonicalBuiltInMainModelParameters(
        rewritten.id,
        assignment.provider,
      );
      ...
      } else if (canonicalMainParameters !== null) {
        rewritten.model_parameters = {
          ...canonicalMainParameters,
          model: assignment.model,
        };
      } else if (
        rewritten.model_parameters &&
...
function buildCanonicalPersistedAgentFields(
  ...
  const canonicalBuiltInMainParameters = canonicalBuiltInMainModelParameters(agent.id, provider);
  ...
  if (canonicalBuiltInParameters !== null) {
    patch.model_parameters = patch.model
      ...
      : canonicalBuiltInParameters;
  } else if (canonicalBuiltInMainParameters !== null) {
    patch.model_parameters = patch.model
      ? {
          ...canonicalBuiltInMainParameters,
          model: patch.model,
        }
      : canonicalBuiltInMainParameters;
  } else {
