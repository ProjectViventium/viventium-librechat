...
class StorageScheduledPromptLifecycleTests(unittest.TestCase):
    ...
    def test_callback_compare_and_swap_cannot_overwrite_newer_terminal_evidence(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = Path(tmpdir) / "schedules.db"
            storage = ScheduleStorage(StorageConfig(db_path=str(db_path)))
            now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
            storage.create_scheduled_prompt_run(
                {
                    ...
                    "result_summary": "Recovery window elapsed.",
            ...
            repaired = storage.update_scheduled_prompt_run_if_current(
                "late-callback-run",
                {
                    "status": "completed",
                    "disposition": "delivered",
                    "completed_at": "2026-08-11T12:01:00Z",
                    "result_summary": "Authoritative late completion.",
                    "error_class": None,
                    "updated_at": "2026-08-11T12:01:00Z",
                },
                expected_status="failed",
                expected_error_class="stale_run_reconciled",
            )
            self.assertTrue(repaired["updated"])
            self.assertEqual(repaired["run"]["status"], "completed")
            self.assertEqual(repaired["run"]["result_summary"], "Authoritative late completion.")

            lost_race = storage.update_scheduled_prompt_run_if_current(
                "late-callback-run",
                {
                    "status": "failed",
                    "disposition": "failed",
                    "error_class": "late_transport_failure",
                    "updated_at": "2026-08-11T12:02:00Z",
                },
                expected_status="failed",
                expected_error_class="stale_run_reconciled",
            )
            self.assertFalse(lost_race["updated"])
            self.assertEqual(lost_race["run"]["status"], "completed")
            self.assertEqual(lost_race["run"]["result_summary"], "Authoritative late completion.")
