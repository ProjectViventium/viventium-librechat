...
from app.config import (
    ...
    EMBEDDINGS_PROVIDER,
    EmbeddingsProvider,
    RAG_CHECK_EMBEDDING_CTX_LENGTH,
    logger,
    vector_store,
    RAG_UPLOAD_DIR,
    CHUNK_SIZE,
...
# RAG embed routes while preserving the default env-configured vector-store path.
# Added: 2026-04-04
RAG_EMBEDDINGS_OPENAI_KEY_HEADER = "x-viventium-embeddings-openai-api-key"
RAG_EMBEDDINGS_OPENAI_BASE_URL_HEADER = "x-viventium-embeddings-openai-base-url"
REQUEST_SCOPED_OVERRIDE_FALLBACK_TOKENS = (
    "api key",
    "authentication",
    "unauthorized",
    "forbidden",
...
def get_vector_store_mode() -> str:
    """Resolve the mode string expected by get_vector_store() for the live store."""
    if isinstance(vector_store, AsyncPgVector):
        return "async"
    if isinstance(vector_store, AtlasMongoVector):
        return "atlas-mongo"
    return "sync"


def build_request_scoped_vector_store(request: Request):
    """Create a temporary vector store when the caller provides request-scoped OpenAI auth."""
    override_api_key = request.headers.get(RAG_EMBEDDINGS_OPENAI_KEY_HEADER)
    if override_api_key is None:
        return None

    override_api_key = override_api_key.strip()
    if not override_api_key:
        return None

    if EMBEDDINGS_PROVIDER != EmbeddingsProvider.OPENAI:
        logger.warning(
            "Ignoring request-scoped OpenAI embeddings override because the active provider is %s",
            EMBEDDINGS_PROVIDER.value,
        )
        return None

    mode = get_vector_store_mode()
    if mode == "atlas-mongo":
        logger.warning(
            "Ignoring request-scoped OpenAI embeddings override because atlas-mongo vector storage is not supported by this override path"
        )
        return None

    override_base_url = request.headers.get(RAG_EMBEDDINGS_OPENAI_BASE_URL_HEADER)
    override_base_url = (
        override_base_url.strip() if override_base_url and override_base_url.strip() else None
    )
    ...
    if mode == "atlas-mongo":
        kwargs["search_index"] = ATLAS_SEARCH_INDEX

    return get_vector_store(**kwargs)


def should_retry_with_default_vector_store(error: Exception) -> bool:
    """Retry with the default store only for likely auth/config override failures."""
    message = str(error).lower()
    return any(token in message for token in REQUEST_SCOPED_OVERRIDE_FALLBACK_TOKENS)
...
async def store_data_in_vector_db(
    ...
    executor=None,
    # === VIVENTIUM START ===
    # Purpose: Support a per-request embeddings override with one-time fallback to the
    # default vector store when the override fails auth/config checks.
    ...
    active_vector_store = target_vector_store or vector_store
    default_vector_store = fallback_vector_store or vector_store

    try:
        if EMBEDDING_BATCH_SIZE <= 0:
            # synchronously embed the file and insert into vector store in one go
            if isinstance(active_vector_store, AsyncPgVector):
                ids = await active_vector_store.aadd_documents(
                    docs, ids=[file_id] * len(docs), executor=executor
                )
            else:
                ids = active_vector_store.add_documents(docs, ids=[file_id] * len(docs))
        else:
            ...
            if isinstance(active_vector_store, AsyncPgVector):
                ids = await _process_documents_async_pipeline(
                    docs, file_id, active_vector_store, executor
                )
            else:
        ...
        if (
            active_vector_store is not default_vector_store
            and should_retry_with_default_vector_store(e)
        ):
            logger.warning(
                "Request-scoped embeddings override failed; retrying with default vector store | File ID: %s | User ID: %s | Error: %s",
                file_id,
                user_id,
                str(e),
            )
            try:
                if EMBEDDING_BATCH_SIZE <= 0:
                    if isinstance(default_vector_store, AsyncPgVector):
                        ids = await default_vector_store.aadd_documents(
                            docs, ids=[file_id] * len(docs), executor=executor
                        )
                    else:
                        ids = default_vector_store.add_documents(
                            docs, ids=[file_id] * len(docs)
                        )
                else:
                    if isinstance(default_vector_store, AsyncPgVector):
                        ids = await _process_documents_async_pipeline(
                            docs, file_id, default_vector_store, executor
                        )
                    else:
                        ids = await _process_documents_batched_sync(
                            docs, file_id, default_vector_store, executor
                ...
                logger.info(
                    "Default vector store fallback succeeded after request-scoped override failure | File ID: %s | User ID: %s",
                    file_id,
                    user_id,
                )
                return {"message": "Documents added successfully", "ids": ids}
                ...
                logger.error(
                    "Default vector store fallback failed after request-scoped override failure | File ID: %s | User ID: %s | Error: %s | Traceback: %s",
                    file_id,
                    user_id,
                    str(fallback_error),
                    traceback.format_exc(),
...
async def embed_local_file(
    ...
    try:
        ...
        request_vector_store = build_request_scoped_vector_store(request)
        result = await store_data_in_vector_db(
            ...
            executor=request.app.state.thread_pool,
            target_vector_store=request_vector_store or vector_store,
            fallback_vector_store=vector_store if request_vector_store is not None else None,
        )
        ...
        if request_vector_store is not None:
            close_vector_store_connections(request_vector_store)
        # Clean up temporary UTF-8 file if it was created for encoding conversion
        if loader is not None:
            cleanup_temp_encoding_file(loader)
