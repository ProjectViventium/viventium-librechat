...
build_image() {
    ...
    # VIVENTIUM START
    # Feature: Title handling + optional Python package expansion at image build time.
    # Why: Keep shell compatibility on older bash versions and enable package extensions without editing requirement files.
    local language_title
    language_title=$(printf '%s' "${language}" | awk '{print toupper(substr($0,1,1)) substr($0,2)}')
    ...
    local docker_build_args=(
        ...
        --label "org.opencontainers.image.title=Code Interpreter ${language_title} Environment"
    ...
    if [ "${language}" = "python" ] && [ -n "${PYTHON_EXTRA_PIP_PACKAGES}" ]; then
        log_info "Applying PYTHON_EXTRA_PIP_PACKAGES during python image build"
        docker_build_args+=(--build-arg "PYTHON_EXTRA_PIP_PACKAGES=${PYTHON_EXTRA_PIP_PACKAGES}")
    fi
    # VIVENTIUM END
...
main() {
    ...
    # VIVENTIUM START
    # Feature: Explicit runtime visibility of optional Python extension channel.
    # Why: Make custom package injection obvious in CI/local logs.
    if [ -n "${PYTHON_EXTRA_PIP_PACKAGES}" ]; then
        log_info "Python extras: ${PYTHON_EXTRA_PIP_PACKAGES}"
    fi
    # VIVENTIUM END
    echo
...
show_help() {
    # VIVENTIUM START
    # Feature: Document optional PYTHON_EXTRA_PIP_PACKAGES build input.
    # Why: Keep operators aware of the extension path exposed by Viventium.
    cat << EOF
...
Environment Variables:
    ...
    PYTHON_EXTRA_PIP_PACKAGES  Space-delimited pip packages applied to python image builds only
